...
interface TransactionMerkleBranch {
  block_height: number;
  merkle: string[];
  pos: number;
}
...
export default class Client {
  ...
  constructor(config: ElectrumConfig) {
    this.electrumClient = new ElectrumClient(config.server, config.port, config.protocol, config.options);
  }
  ...
  async latestBlockHeight(): Promise<number> {
    ...
    const header = await this.electrumClient.blockchain_headers_subscribe().catch((err) => {
  ...
  async getTransaction(txHash: string): Promise<TransactionData> {
    const tx = await this.electrumClient.blockchain_transaction_get(txHash, true).catch((err) => {
  ...
  async broadcastTransaction(rawTX: string): Promise<string> {
    const txHash = await this.electrumClient.blockchain_transaction_broadcast(rawTX).catch((err) => {
   ...
   */
  async getUnspentToScript(script: string): Promise<UnspentOutputData[]> {
    const scriptHash = scriptToHash(script);

    const listUnspent = await this.electrumClient.blockchain_scripthash_listunspent(scriptHash).catch((err) => {
   ...
   *         decimal strings.
   */
  async getBalanceOfScript(script: string): Promise<{ confirmed: string; unconfirmed: string }> {
    const scriptHash = scriptToHash(script);

    const balance = await this.electrumClient.blockchain_scripthash_getBalance(scriptHash).catch((err) => {
      throw new Error(JSON.stringify(err));
    });
   ...
   */
  async onTransactionToScript<T>(script: string, callback: any): Promise<T> {
    const scriptHash = scriptToHash(script);

    // Check if transaction for script already exists.
    const initialStatus = await this.electrumClient.blockchain_scripthash_subscribe(scriptHash).catch((err) => {
    ...
    if (result) {
      ...
      await this.electrumClient.blockchain_scripthash_unsubscribe(scriptHash).catch((err) => {
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        const { electrumClient } = this;

        // eslint-disable-next-line consistent-return
        const listener = async (/** @type {[string, string]} */ msg: [string, string]) => {
          ...
          if (receivedScriptHash === scriptHash) {
            ...
            if (result) {
              ...
              await electrumClient.blockchain_scripthash_unsubscribe(scriptHash).catch((err) => {
                throw new Error(`failed to unsubscribe: ${err}`);
              });
   ...
   * @return {Promise<T>} Value resolved by the callback.
   */
  async onNewBlock<T>(callback: any): Promise<T> {
    // Subscribe for new block notifications.
    const blockHeader = await this.electrumClient.blockchain_headers_subscribe().catch((err) => {
      throw new Error(`failed to subscribe: ${err}`);
    });
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        // eslint-disable-next-line consistent-return
        const listener = async function (/** @type {NewBlockData[]} */ messages: NewBlockData[]) {
   ...
   */
  async getMerkleRoot(blockHeight: number): Promise<Buffer> {
    const header = await this.electrumClient.blockchain_block_header(blockHeight, null).catch((err) => {
      throw new Error(`failed to get block header: [${err}]`);
    });
   ...
   */
  async getHeadersChain(blockHeight: number, confirmations: number): Promise<string> {
   ...
   *         hexadecimal form.
   */
  async getTransactionMerkle(txHash: string, blockHeight: string): Promise<TransactionMerkleBranch> {
    return this.electrumClient.blockchain_transaction_getMerkle(txHash, blockHeight).catch((err) => {
   ...
   */
  async findOutputForAddress(txHash: string, address: string): Promise<number> {
   ...
   */
  async getTransactionsForScript(script: string): Promise<TransactionData[]> {
    const scriptHash = scriptToHash(script);
    /** @type {{ height: number, tx_hash: string }[]} */
    const history: { height: number; tx_hash: string }[] = await this.electrumClient.blockchain_scripthash_getHistory(
      scriptHash
    );
    ...
    const filteredTransactions: TransactionData[] = transactions.filter((tx) => tx);
 ...
 */
function scriptToHash(script: string): string {
  /** @type {Buffer} */
  const scriptHash: Buffer = digest(Buffer.from(script, 'hex')).reverse();
