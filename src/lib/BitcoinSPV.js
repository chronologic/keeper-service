...
const BcryptoMerkle = require('bcrypto/lib/merkle.js');
...
class BitcoinSPV {
  ...
  async getTransactionProof(txHash, confirmations) {
    ...
    const latestBlockHeight = await this.client.latestBlockHeight().catch((err) => {
    ...
    // GET HEADER CHAIN
    const headersChain = await this.client.getHeadersChain(txBlockHeight, confirmations).catch((err) => {
      throw new Error(`failed to get headers chain: [${err}]`);
    });

    // GET MERKLE PROOF
    const merkleProofInfo = await this.getMerkleProofInfo(txHash, txBlockHeight).catch((err) => {
  ...
  async verifyMerkleProof(proofHex, txHash, index, blockHeight) {
    ...
    const actualRoot = await this.client.getMerkleRoot(blockHeight).catch((err) => {
