...
export async function ensureRedemptionRequested(deposit: Deposit): Promise<Deposit> {
  ...
  try {
    ...
    const tx = await requestRedemption(deposit);
...
async function requestRedemption(deposit: Deposit): Promise<IEthTx> {
